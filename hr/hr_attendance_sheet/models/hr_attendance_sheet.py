...
class AttendanceSheet(models.Model):
    ...
    def action_confirm_payslips(self):
        for i in self:
            move = i.payslip_id.move_id.id
            if move:
                journal = self.env['account.move'].search([('id', '=', move)]).id
                if move == journal:
                    break
                elif move != journal:
    ...
    def open_payment_wizard(self):
        """
        This Function opens a wizard from Payment Wizard with default data
        """
        return {'name': _('New Payslip Payment'),
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'payment.wizard',
            'views': [(self.env.ref('hr_attendance_sheet.payment_wizard_view').id, 'form')],
            'view_id': self.env.ref('hr_attendance_sheet.payment_wizard_view').id,
            'target': 'new',}
    ...
    def create_payslip(self):
        ...
        for att_sheet in self:
            ...
            att_sheet.payslip_id.compute_sheet()
...
class HrPayslip(models.Model):
    ...
    check = fields.Boolean(string = 'check attenndas')
    attendance_sheet = fields.Many2many('attendance.sheet',string='Attendance')
    def get_worked_day_lines(self, contract_ids, date_from, date_to):
        res = super(HrPayslip, self).get_worked_day_lines(contract_ids, date_from, date_to)
        contract_obj = self.env['hr.contract']
        no_overtime = 0
        tot_overtime =0
        no_absence = tot_absence =0
        no_late = tot_late =0
        no_difftime = tot_difftime=0


        att_sheet = self.env['attendance.sheet'].search([('employee_id', '=', self.employee_id.id),  ('date_from', '>=',date_from ), ('date_to', '<=', date_to),('state', '=', 'waiting_payment')])
        self.attendance_sheet=att_sheet

        for obj in att_sheet:
            if obj:
                no_overtime += obj.no_overtime
                tot_overtime += obj.tot_overtime
                no_absence += obj.no_absence
                tot_absence += obj.tot_absence
                no_late += obj.no_late
                tot_late += obj.tot_late
                no_difftime += obj.no_difftime
                tot_difftime += obj.tot_difftime
        ...
        res.append(difftime)
        if difftime or overtime or late or absence:
            self.check = True
    ...
    def action_payslip_done(self):
        for line in  self.attendance_sheet:
            if self.check == True and line:
